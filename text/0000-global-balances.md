...
This RFC defines a global address-balance map that co-exists with UTXOs. The new balance domain is keyed by `(address, token)` and is intended to be the standard shared balance space for user-owned funds that contracts use. Balances in this domain are stored exclusively with [Token Amount V2](../projects/token-amount-v2/0002-token-amount-v2.md) precision — 18-decimal base-unit integers — with no 2-decimal (V1) representation anywhere in the design.
...
This feature is built on top of [Token Amount V2](../projects/token-amount-v2/0002-token-amount-v2.md) (proposed in [PR #115](https://github.com/HathorNetwork/rfcs/pull/115)), the upgrade from 2 to 18 decimal places. Every amount in the global map is an 18-decimal (V2) base-unit integer: the examples below write human-readable values such as `4 HTR`, but on the wire and in storage that is `4 * 10^18` base units. The global balance map is 18-decimal native; it does not exist for the legacy 2-decimal (V1) representation. See [Decimal precision](#decimal-precision-token-amount-v2) for the full rules.
...
- On the wire, transfer amounts use the V2 token-amount codec: a 1-byte length-prefix varint (Token Amount V2, B6/I1). Token Amount V2 already establishes that a transaction's version dictates the version of all its token-amount fields — output values, fee values, and nano action values (I5). This RFC extends that list with `TxTransferInput.amount` and `TxTransferOutput.amount`.
- Global balance and seqnum entries live in the same patricia-trie used for nano contract state, which Token Amount V2 re-serializes to V2 (I18). Amounts are stored using the trie's existing internal integer encoding (LEB128); this RFC introduces no new encoding. Note that a stored balance accumulates credits across many transactions, so it is not subject to the per-output cap of `2^63 * 10^16` (I2): the variable-length encoding accommodates larger sums, and total token supply is the practical bound.
...
- Token Amount V2 already requires a sync from scratch and breaks the nano state root-id (I18/I19). Adding the two new tables to the trie therefore introduces no additional migration burden beyond V2's own.
...
`amount` fields are encoded with the same versioned token-amount codec used by output values and the other headers: the transaction's token-amount version dictates the version of all its token-amount fields (Token Amount V2, I5), and this RFC adds transfer amounts to that list. Because this header only exists in the V2 era (see [Decimal precision](#decimal-precision-token-amount-v2)), **a `TransferHeader` is only valid on a V2 transaction** — one whose token-amount version bit is set in `flags_byte` (Token Amount V2, I4). A V1 transaction carrying a `TransferHeader` is invalid. Each `amount` is therefore an 18-decimal base-unit integer encoded as a 1-byte length-prefix varint (B6/I1), bounded by the same maximum output value as regular outputs, `2^63 * 10^16` (I2).
...
- only transactions may carry it (blocks remain V1-only under Token Amount V2 (I3), so they could not carry one anyway)
- the carrying transaction must be a V2 transaction (its token-amount version bit set in `flags_byte`, Token Amount V2, I4); a `TransferHeader` on a V1 transaction is rejected
