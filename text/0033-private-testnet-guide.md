...
To run it, you need to build the Docker image using the Dockerfile in the repo. For the `<address>`, you can use the same you generated in the beginning for the genesis block:
```sh
docker build -t tx-mining-service .

docker run -it --network="host" tx-mining-service http://127.0.0.1:8083 --address <address> --api-port 8100 --stratum-port 8101 --testnet
```
